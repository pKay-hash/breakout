...
function PlayState:enter(params)
    ...
    self.numLocked = params.numLocked
...
function PlayState:update(dt)
    ...
    if self.powerupTimer >= 0 then
        ...
        p = Powerup(math.random(0, VIRTUAL_WIDTH - 16), self.paddle.y - 100, (self.numLocked > 0) and (not self.hasKeyPowerup)
        )
    ...
    for y, powerup in pairs(self.powerups) do
        powerup:update(dt)
    ...
    for k, ball in pairs(self.balls) do
		...
		for i, brick in pairs(self.bricks) do
			...
			if brick.inPlay and ball:collides(brick) then
				...
				if brick.lcked == true and self.hasKeyPowerup then
					self.score = self.score + 1000
					brickUnlocked = true
				elseif brick.lcked == true then
				...
				if brickUnlcked then
					self.hasKeyPowerup = false
					self.numLocked = self.numLocked - 1
				end
...
function PlayState:powerUpSpawn()
